<script>
    ...
    async function handleSubmit(e) {
      ...
      const hashedPassword = await hashPassword(user.password.value, salt);
      const { publicKey, privateKey } = await createKeyPair();
      const signature = await signPublicKey(privateKey, publicKey);
      await storeKeys(user.username.value, publicKey, signature);
